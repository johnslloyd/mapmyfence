...
import { ArrowLeft, Printer, ClipboardCheck, ExternalLink } from "lucide-react";
...
// Selection state is per (project, store), local to this device. Not a
// "have I gathered this" tracker — see the note above the Select-all
// row below for why that was dropped. Persisted the same way
// pendingFenceLine is: no other viewer needs to see it, so localStorage
// over a schema addition.
function storageKey(projectId: number, store: string) {
  return `shoppingListSelected:${projectId}:${store}`;
}

function useSelectedItems(projectId: number, store: string) {
  ...
  // Functional updates throughout — not `{ ...selected, [id]: ... }` off
  // the outer closure. Two toggles fired in the same tick (as happened
  // in testing: two checkbox clicks dispatched back-to-back before React
  // re-rendered between them) would otherwise both read the same stale
  // `selected` snapshot and the second call would silently clobber the
  // first instead of composing with it.
  const persist = (next: Record<number, boolean>) => {
    try {
      localStorage.setItem(key, JSON.stringify(next));
    } catch {
      // Best-effort — a full/blocked localStorage just means selection
      // doesn't persist across reloads, not a broken page.
    }
  };
...
function ShoppingListContent({ projectId, isGuest }: { projectId: number; isGuest: boolean }) {
  ...
  const { selected, toggle, selectAll, clear } = useSelectedItems(projectId, active?.store ?? "none");
  ...
  const selectedCount = materials.filter((m) => selected[m.id]).length;
  const selectedWithLinks = materials.filter((m) => selected[m.id] && m.url);

  const openSelected = () => {
    for (const item of selectedWithLinks) {
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 md:p-8 space-y-6">
      ...
      <div className="bg-panel text-panel-foreground rounded-lg border border-border/50 shadow-sm p-4 md:p-6 space-y-6">
        ...
        {/* Selecting items isn't a "gathered this" tracker — printing this
            page is how most people take it to the store, and printed
            checkboxes below (blank, independent of on-screen selection)
            cover that. Selecting here is for the one thing selection can
            actually DO today: open every selected item's product page at
            once. It's also the natural precursor to a real "send this as
            a batch order" action later — this is deliberately just that
            one small, real, un-scoped-creep step toward it. */}
        <div className="flex items-center justify-between gap-3 print:hidden">
          <div className="flex items-center gap-3 text-sm">
            <button
              className="text-primary hover:underline"
              onClick={() => selectAll(materials.map((m) => m.id))}
            >
              Select all
            </button>
            <span className="text-muted-foreground">·</span>
            <button className="text-primary hover:underline" onClick={clear}>
              Clear
            </button>
            <span className="text-muted-foreground">{selectedCount} selected</span>
          </div>
          <Button size="sm" className="gap-2" disabled={selectedWithLinks.length === 0} onClick={openSelected}>
            <ExternalLink className="w-4 h-4" /> Open {selectedWithLinks.length || ""} online
          </Button>
        </div>
        ...
        <div className="space-y-6">
          {grouped.map((group) => (
            <div key={group.type} className="space-y-2">
              ...
              <div className="divide-y rounded-lg border border-border/50 bg-card/40">
                {group.items.map((item) => (
                  <label
                    key={item.id}
                    className={cn(
                      "flex items-center gap-3 p-3 cursor-pointer transition-colors hover:bg-primary/5",
                      selected[item.id] && "bg-primary/5"
                    )}
                  >
                    <Checkbox checked={!!selected[item.id]} onCheckedChange={() => toggle(item.id)} className="print:hidden" />
